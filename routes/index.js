...
client.on('message', function (topic, message) {
    ...
    const msgStr = message.toString();
        ...
        }else if(topicParts[2] === 'MODIFY'){
            const modifiedItem = JSON.parse(msgStr);
            liste.modifyItem(modifiedItem.id, modifiedItem.nom, modifiedItem.prix);
        }
        socket.getIO().emit('updateItems', liste.getItems());
});
router.post("/modify", function (req, res) {
    const id = parseInt(req.body.id, 10);
    const newName = req.body.nom.trim();
    const newPrice = parseFloat(req.body.prix);

    if (id && newName && !isNaN(newPrice)) {
        liste.modifyItem(id, newName, newPrice);
        console.log(`Item avec ID "${id}" modifié.`);
        client.publish('ITEM/WEB/MODIFY', JSON.stringify({ id, nom: newName, prix: newPrice }));
    }
